...
//Pagination
function vezaconslt_the_pagination2($args = array(), $echo = 1)
{
	...
	$pagination = str_replace("<ul class='page-numbers'", '<ul class="styled-pagination style2 text-center clearfix"', paginate_links($args) );
...
add_filter('vezaconslt_redux_custom_fonts_load', 'vezaconslt_redux_custom_fonts_load');
function vezaconslt_redux_custom_fonts_load( $custom_font ) {
	$custom_style = '';
	$pathinfo = pathinfo( $custom_font );
	if ( $filename = vezaconslt_set( $pathinfo, 'filename' ) ) {
		$custom_style .= '@font-face{
            font-family:"' . $filename . '";';
		$extensions   = array( 'eot', 'woff', 'woff2', 'ttf', 'svg' );
		$count        = 0;
		foreach ( $extensions as $extension ) {
			$file_path = esc_url(home_url('/')) . '/wp-content/themes/vezaconslt/assets/css/custom-fonts/' . $filename . '.' . $extension;
...
/**
 * [vezaconslt_social_share_output description]
 *
 ...
 */
function vezaconslt_social_share_output( $icon, $color = false ) {
...
add_action('vezaconslt_social_share_output', 'vezaconslt_social_share_output');

function vezaconslt_share_us($PostID = '', $PostName = '')
{
...
function vezaconslt_get_social_icons()
{
    ...
    <?php $icons = $options->get( 'icons_social_share' );
		...
		<a href="<?php echo esc_url(vezaconslt_set( $header_social_icons, 'url' )); ?>" style="background-color:<?php echo esc_attr(vezaconslt_set( $header_social_icons, 'background' )); ?>; color: <?php echo esc_attr(vezaconslt_set( $header_social_icons, 'color' )); ?>"><i class="fab <?php echo esc_attr( vezaconslt_set( $header_social_icons, 'icon' ) ); ?>"></i></a>
		<?php endforeach; ?>
	<?php endif; ?>
<?php }

function vezaconslt_get_social_icons2()
{
	$options = vezaconslt_WSH()->option(); ?>
    ...
    <?php $icons = $options->get( 'icons_social_share' );
		...
		<ul class="social-links clearfix">
			...
			$icon_class = explode( '-', vezaconslt_set( $header_social_icons, 'icon' ) ); ?>
			<li><a href="<?php echo esc_url(vezaconslt_set( $header_social_icons, 'url' )); ?>" style="background-color:<?php echo esc_attr(vezaconslt_set( $header_social_icons, 'background' )); ?>; color: <?php echo esc_attr(vezaconslt_set( $header_social_icons, 'color' )); ?>"><i class="fab <?php echo esc_attr( vezaconslt_set( $header_social_icons, 'icon' ) ); ?>"></i></a></li>
			<?php endforeach; ?>
		</ul>
	<?php endif; ?>
<?php }

function vezaconslt_get_post_view() {
    $count = get_post_meta( get_the_ID(), 'post_views_count', true );
    return "$count views";
}

function vezaconslt_set_post_view() {
...
function vezaconslt_posts_column_views( $columns ) {
    $columns['post_views'] = 'Views';
    return $columns;
}

function vezaconslt_posts_custom_column_views( $column ) {
    if ( $column === 'post_views') {
        echo vezaconslt_get_post_view();
    }
}
add_filter( 'manage_posts_columns', 'vezaconslt_posts_column_views' );
add_action( 'manage_posts_custom_column', 'vezaconslt_posts_custom_column_views' );
